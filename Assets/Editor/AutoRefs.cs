...
public class AutoRefs : Editor
{
			...
			for (int nMonoBehaviour = 0; nMonoBehaviour < acMonoBehaviours.Length; nMonoBehaviour++)
			{
				...
				for (int nFieldInfo = 0; nFieldInfo < acFieldInfos.Length; nFieldInfo++)
				{
					...
					if (cAutoRef != null)
					{
						...
						List<GameObject> lstAutoRefTargets = GetAutoRefTargets(cGameObject, cAutoRef.m_eTargetType);
	...
	static List<GameObject> GetAutoRefTargets(GameObject cGameObject, AutoRefTargetType eTargetType)
	{
